...
const UserProfile = ({ user, onClose, calculateAge }) => {
  const profileRef = useRef(null); // Referință către containerul modalului

  // Detectăm clic-urile în afara profilului
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (profileRef.current && !profileRef.current.contains(event.target)) {
        onClose(); // Închide profilul
      }
    };
      ...
      <div ref={profileRef} className="modal-content user-profile">
        ...
        <div className="profile-header">
          <div className="profile-avatar">
            {user.playerId?.image ? (
              <img src={`http://localhost:5000${user.playerId.image}`} alt="Profile" className="profile-image" />
            ) : (
